...
{
  public class ProviderTwoConsumerService : BackgroundService
  {
    const int kafka_consume_exception_id = 502;
    const int unhandled_exception_id = 500;

    private const string ProviderId = "provider-two";

    private readonly ILogger<ProviderTwoConsumerService> _logger;
    ...
    public ProviderTwoConsumerService(IOptions<KafkaConfig> kafkaOptions,
      ILogger<ProviderTwoConsumerService> logger, IServiceProvider services)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
      try
      {
        ...
        var gateway = _services.GetRequiredKeyedService<IRoutesProviderGateway>("ProviderTwoGateway");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
          ...
          try
          {
            //_providerRoutesUpdateRequestedConsumer.StoreOffset(consumeResult);
          }
    ...
    // todo: extract from business logic
